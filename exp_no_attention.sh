...
model_config="--model_type $model --hidden_size 256 --num_layers 1 --emb_size 256 --z_dim $z_dim --init_type normal --use_projection --use_attention 0"
...
train_config="--epochs 8 --batch_size 64 --print_every 1000 --dropout .50" #vnmt technically did not use dropout, but it does help with generalization 
...
then
	...
	outfile="$out"-"no-attn"-"$((i + 1))".out # out file name
	sbatch --output "$outfile" exp_no_attention.sh $((i + 1)) $out  $model $kl_anneal $z_dim $flow_type $num_flows $num_particles
fi
